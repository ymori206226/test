...
def openfermion_print_state(state,n_qubit,j_state):
    ...
    for i in range(2**n_qubit):
        v = state[i][j_state]
        if abs(v)**2>0.01:
            prints('|',format(i,opt),'> : ', '{a.real:+.4f} {a.imag:+.4f}i'.format(a=v))
    ...
    for a in range(nva):
        ...
        for i in range(noa):
            ...
            if abs(theta_list[ia]) > threshold:
                prints(ii, "a -> ", aa, "a  : ", '%2.10f' % theta_list[ia],filepath=filepath)
            ia = ia + 1
    for a in range(nvb):
        aa = a + 1 + nob
        for i in range(nob):
            ii = i + 1 
            if abs(theta_list[ia]) > threshold:
                prints(ii, "b -> ", aa, "b  : ",  '%2.10f' % theta_list[ia],filepath=filepath)
            ia = ia + 1
            ...
            for j in range(noa):
                ...
                for i in range(j):
                    ...
                    if abs(theta_list[ijab]) > threshold:
                        prints(ii, "a", jj,"a -> ", aa, "a", bb,"a  : ", '%2.10f' % theta_list[ijab], filepath=filepath)
                    ijab = ijab + 1
        ...
        for a in range(min(b+1,nva)):
            ...
            for j in range(nob):
                ...
                for i in range(j+1):
                    ...
                    # b > a, j > i
                    if abs(theta_list[ijab]) > threshold:
                        prints(ii, "a", jj,"b -> ", aa, "a", bb,"b  : ", '%2.10f' % (theta_list[ijab]), filepath=filepath)
                    ijab = ijab + 1
        ...
        for a in range(b):
            ...
            for j in range(nob):
                ...
                for i in range(j):
                    ii = i + 1
                    if abs(theta_list[ijab]) > threshold:
                        prints(ii, "b", jj,"b -> ", aa, "b", bb,"b  : ", '%2.10f' % (theta_list[ijab]), filepath=filepath)
                    ijab = ijab + 1
...
def print_amplitudes_spinfree(theta_list,no,nv,threshold=0.01,filepath=cf.log):
    ...
    for a in range(nv):
        ...
        for i in range(no):
            ii = i + 1 
            if abs(theta_list[ia]) > threshold:
                prints(ii, " -> ", aa, "  : ", '%2.10f' % theta_list[ia],filepath=filepath)
            ia += 1
    ### print doubles amplitudes ###
    for b in range(nv):
        bb = b + 1 + no
            ...
            for j in range(no):
                ...
                for i in range(j):
                    ...
                    if abs(theta) > threshold:
                        prints(ii, "a", jj,"a -> ", aa, "a", bb,"a  : ", '%2.10f' % theta, filepath=filepath)
                        prints(ii, "b", jj,"b -> ", aa, "b", bb,"b  : ", '%2.10f' % theta, filepath=filepath)
            ...
            for j in range(no):
                ...
                for i in range(j+1):
                    ...
                    if abs(theta_list[baji]) > threshold:
                        prints(ii, "a", jj,"b -> ", aa, "a", bb,"b  : ", '%2.10f' % (theta_list[baji]), filepath=filepath)
    ...
    if dimension == 2:
        ...
        while imax < m:
            ...
            for j in range(n):
                ...
                for i in range(imin-1,imax):
                    prints("  {v: 12.7f}  ".format(v=A[j][i]), end="", filepath=filepath)
                prints("",filepath=filepath)
    elif dimension == 1:
        if(n==None or m==None):
            if(type(A) is list):
        ...
        while imax < m:
            ...
            for j in range(n):
                prints(" {J:4}  ".format(J=j),end="",filepath=filepath)
                for i in range(imin-1,imax):
                    prints("  {v: 12.7f}  ".format(v=A[j+i*n]), end="", filepath=filepath)
                prints("",filepath=filepath)
