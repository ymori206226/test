...
def test_observable(state, obs, obsZ, n_sample):
    ...
    for i in range(n_term):
        ...
        qulacs_measure_observable = create_observable_from_openfermion_text(
        ...
        expH += coef[i] * prob
        print("coef: ", "%10.5f" % coef[i], "   prob: ", "%10.5f" % prob)
    return expH


def sample_observable_noisy_circuit(
    circuit, initial_state, obs, n_circuit_sample=1000, n_sample_per_circuit=1
):
    """Function
    Args:
            ...
            for i in range(Ng):
                ...
                print(
                    test_transition_observable(
                        state_g, qulacs_hamiltonianZ, poststate0, poststate1, 100000
                    )
                )
                # exit()
                ...
                HUg.append(
                    sample_observable(state_g, qulacs_hamiltonianZ, i_sample_x).real
                )
                ...
                Ug.append(
                    sample_observable(state_g, qulacs_ancZ, i_sample_x * n_term).real
                )
                # p0_sample = 0
    ...
    for i_sample in samplelist:
        ...
        for icyc in range(ncyc):
            ...
            for i in range(Ng):
                ...
                ### Compute expectation value <HUg> ###
                HUg.append(
                    sample_observable(state_g, qulacs_hamiltonianZ, i_sample).real
                )
                ### <S2Ug> ###
