...
from .ucclib import ucc_singles, set_circuit_uccsd, set_circuit_uccd, set_circuit_sauccd, single_ope_Pauli
from .fileio import SaveTheta, LoadTheta, print_state, print_amplitudes, prints, printmat
def set_projection():
    """ Function
    ...
    """
    prints("Projecting to spin space :  s = {:.1f}    Ms = {} ".format((cf.spin-1)/2,cf.Ms))
    prints("             Grid points :  (alpha,beta,gamma) = ({},{},{})".format(cf.euler_ngrids[0],cf.euler_ngrids[1],cf.euler_ngrids[2]))

    trap = True
    # Trapezoidal or Simpson for alpha, gamma 
    if cf.euler_ngrids[0] > 0:
        if trap:
...
#    print("Time: ",t2-t1)
    ...
    Ep = 0
    for ibeta in range(nbeta):
        Ep += beta_coef * HUg[ibeta] / Norm
        S2 += beta_coef * S2Ug[ibeta] / Norm
    t2 = time.time() 
    cpu1 = t2 - t1
    Ep += coef0_H
    S2 += coef0_S2
...
def S2Proj_test(qulacs_s2):
    ...
    state_P.normalize(norm2)
    print_state(state_P,name="P|Q>",threshold=1e-6) 
    prints("<S**2> = {: 17.15f} ".format(qulacs_s2.get_transition_amplitude(state_P,state_P)))
    print("<S**2> = {: 17.15f} ".format(qulacs_s2.get_transition_amplitude(state_P,state_P)))
    exit()
    return state_P
